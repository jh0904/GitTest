...
public class NodeTest {
    ...
    public void addNode(Node node) {
        ...
        while (temp.next != null) {    //遍历单链表，直到遍历到最后一个则跳出循环。
            temp = temp.next;        //往后移一个结点，指向下一个结点。找出最后一个节点（最后一个节点next为null）
        }
     ...
     * 记录要插入节点的上一个节点，修改其next,然后再把插入的节点的next修改成上一个节点的next。
     * index:插入链表的位置，从1开始
     * node:插入的结点
     */
    ...
    public static void main(String[] args) {
        ...
        n.addNode (new Node (11));
        n.addNode (new Node (22));
        n.addNode (new Node (33));
        n.print ();
        System.out.println (n.length ());

        System.out.println ("----------------");
        n.addNodeHead (new Node (44));
        n.selectSortNode ();
        n.print ();
    }
}
